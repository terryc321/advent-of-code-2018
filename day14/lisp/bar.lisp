...
#|
	(let ((fc (fcheck3 #(0 1 2 4 5) step len xs)))
	  (when fc
	    (format t "01245 occurs at index ~a .. ~%" fc)
	    (fshow2 step len xs a b)
	    (break)
	    ))
|#
(defmacro one-shot (v)
  (let ((fired (gensym "fired"))
	(fc (gensym "fc")))
  `(progn
     (let ((,fired nil))
       (lambda ()
	 (when (not ,fired)
	   (let ((,fc (fcheck3 ,v step len xs)))
	     (when ,fc
	       (format t "~a occurs at index ~a ~%" ,v ,fc)
	       (setq ,fired t)
	       (break)
	       ))))))))
...
(defun f3 ()
  (let* ((xs (let ((h (make-hash-table :test #'eql)))
	 ...
	 (len 2)
	 (end 1)
	 (shots (list (one-shot #(5 1 5 8 9))
		      (one-shot #(0 1 2 4 5))
		      (one-shot #(9 2 5 1 0))
		      (one-shot #(5 9 4 1 4))
		      (one-shot #(7 6 8 0 7 1))))
	 )
    ...
    (loop
	 ...
	 (let* ((ai (gethash a xs))
	   ...
	   ;; (let ((fc (fcheck3 #(5 1 5 8 9) step len xs)))
	   ;;   (when fc
	   ;;     (format t "51589 occurs at index ~a .. ~%" fc)
	   ;;     (fshow2 step len xs a b)
	   ;;     (break)
	   ;;     ))
	   ...
	   ;; 	)
    	   ...
    	   ;; ;; look for sequences after made 10......00 gazillion 
	   ;;   (dolist (shot shots)
	   ;;     (funcall shot))
    ))
...
;; 
;;(sb-ext:dynamic-space-size)       1073741824      ;; 1024 ??
...
given some longer sequence .......
check if somewhere in there 5 1 5 8 9 appears in that order
|#
(defun fcheck3 (v step len xs)
  (let ((vlen (length v)))
    (catch 'found
      (loop for i from 0 to (- len 1) do
	(catch 'no
	  (loop for j from 0 to (- vlen 1) do
	    (when (not (equalp (aref v j)
			       (gethash (+ i j) xs)))
	      (throw 'no nil)))
	  (throw 'found i))))))
...
needed to double dynamic-space-size to 2048  ( 2 gigs ?? )

#(5 1 5 8 9) occurs at index 9 
#(0 1 2 4 5) occurs at index 5 
#(9 2 5 1 0) occurs at index 18 
#(5 9 4 1 4) occurs at index 2018 
...
...
step 15500000 
#(7 6 8 0 7 1) occurs at index 20198090 

20198090  ACCEPTED answer !
