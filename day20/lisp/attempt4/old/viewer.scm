...
(define *score-board* (make-2d-array 2400 2400 #f))
...
(define (bfs x y n)
  ...
  (let ((ch1 (safe-board-ref *board* x (+ y 0)))
    ...
    (let ((val (safe-score-ref x y)))      
      (when (or (not val) (< n val))
	(safe-score-set! x y n)))    
...
(define (find-highest)
  (let ((highest 0)(highest-pt #f))
	...
	((>= y 2400) #f)
      (do ((x 1 (+ x 1)))
	  ((>= x 2400) #f)
	(let* ((val (array-ref2 *score-board* x y)))
	  (cond
	   ((not val) #f) ;; a wall
	   ((> val highest)
	    (set! highest val)
