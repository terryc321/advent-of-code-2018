...
(define (trav-symbol t pts)
  (let* ((s (format #f "~a" t))
    ...
    (cond
       ((string=? s "?") pts) ;nop
       (#t (trav-symbol-parallel s 0 slen pts)))))


(define (trav-symbol-parallel s i slen pts)
  (cond
   ((>= i slen) pts)
   (#t
    (trav-symbol-parallel s (+ i 1) slen
			  (map (lambda (pt)
				 (let ((x (point-x pt))(y (point-y pt)))
				   (let ((ch (string-ref s i)))
				     (cond
				      ;; north
				      ((char=? ch #\N)
				       (open-trap-door! x (+ y 1)) ;; record #\_ at x (y + 1)
				       (mark-board! x y #\.)
				       (mark-board! x (+ y 2)  #\.)
				       (make-point x (+ y 2)))
				      ;; south				       
				      ...
				      ;; east
				      ((char=? ch #\E)
				       (open-side-door! (+ x 1) y) ;; record #\| at (x + 1) y
				       (mark-board! x y #\.)
				       (mark-board! (+ x 2) y #\.)
				       (make-point (+ x 2) y))
				      ;; west
				      ((char=? ch #\W)
				       ...
				       (mark-board! (+ x -2) y #\.)
				       (make-point (+ x -2) y))
				      (#t (error (format #f "trav-symbol-parallel : error letter ~a not handled" ch)))))))
			       pts)))))
