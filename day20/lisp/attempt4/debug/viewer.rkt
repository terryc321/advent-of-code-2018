...
(define (safe-board-ref bd x y)
  (let ((tx (translate-in-x x))
	(ty (translate-in-y y)))    
  ;; (format2 #t "safe-array-ref2 BD ~a ~a~%" tx ty)
  (cond
   ((onboard? tx ty) (array-ref2 bd tx ty))
   (#t #f))))
	    
;; (bfs-help working-stack accumulator-stack  current-step 
(define (bfs-help working accumulator step)
  (cond
   ((null? working) accumulator)
...
;; step 
(define (bfs)
  (safe-board-set! *board*    
      
  "breadth first search"
  ;; NORTH
  (dmesg #t "bfs : ~a ~a ~a : at north." x y n)
  (let ((ch1 (safe-board-ref *board* x (+ y 0)))
    ...
    (when (and ch1 ch2 ch3
      ...
      (bfs x (+ y 2) (+ n 1))))
  ;; SOUTH
  (dmesg #t "bfs : ~a ~a ~a : at south." x y n)
  (let ((ch1 (safe-board-ref *board* x (+ y 0)))
	(ch2 (safe-board-ref *board* x (+ y -1)))
	(ch3 (safe-board-ref *board* x (+ y -2))))
    (when (and ch1 ch2 ch3
      ...
      (bfs x (+ y -2) (+ n 1))))
  ;;EAST
  (dmesg #t "bfs : ~a ~a ~a : at east." x y n)
  ...
  ;; WEST
  (dmesg #t "bfs : ~a ~a ~a : at west." x y n)
  (let ((ch1 (safe-board-ref *board* (+ x 0) y))
	(ch2 (safe-board-ref *board* (+ x -1) y))
	(ch3 (safe-board-ref *board* (+ x -2) y)))
    (when (and ch1 ch2 ch3
