...
;; alt contains nil ?
;; maybe we should push alt back on stack and pop2 off ?
;;
...
(defun parse-word()
  ...
  (cond
    ((dir-p)
	 ...
	 (advance))
       (push-stack (concatenate 'string (reverse chars)))))
...
;; nsew | news => means all of nsew OR news 
;; a | b | c | d ... 
(defun parse-alt()
  (parse-parens)
  ;; (cond
  ;;   ((dir-p) (parse-word))
  ;;   (t (format t "parse-alt top: what is this character ? => ~a~%" (cur))
  ;;      (error "wtf parse-alt")))
  (cond    
    ((alt-p)
     (let ((a (car stack)))
       (setq stack (cdr stack))
       (when (null a) (format t "how can A be nil ?~%")(error "a is nil - parse alt"))
       (let ((alts (list a)))
    ...
    ;; (t (format t "parse-alt bot : what is this character ? => ~a~%" (cur))
    ;;    ;; (error "wtf parse-alt")
    ;;    )))
...
(defparameter *output* (run *input*))


;; can we try to confirm by generating a string
(defun gen (tree)
  (cond
    ((stringp tree) ;; print ? also ; as adjusted file has ? also 
     (format t "~a" tree))
    ((and (listp tree) (eq (car tree) 'alt))
     (loop for i from 1 to (- (length tree) 1) do
       (let ((elem (nth i tree)))
	 (gen elem)
	 (when (not (= i (- (length tree) 1)))
	   (format t "|")))))
     ;; (cond
     ;;   ((= (length tree) 3) ;; 2 legs
     ;; 	(format t "ALT-2LEG"))
     ;;   (t
     ;; 	(format t "ALT-~aLEGS" (-(length tree)1))
     ;; 	)))
    ((and (listp tree) (eq (car tree) 'seq))
     (format t "(")
     (loop for i from 1 to (- (length tree) 1) do
       (let ((elem (nth i tree)))
	 (gen elem)))
	...
	(loop while t do
	  (let ((a (read-char stream))
	    ...
	    (cond	      
	      ((char= a b)
	       (incf good)
	       (format t "OK at ~a : ~a : ~a ~%" i a b))
	      (t
	       (incf bad)
	       (format t "BAD at ~a : gen ~a  :  ~a ~%" i a b)))
	    (incf i)))))))
...
(defun save-gen()
  (with-open-file (stream "output/generated.lisp" :direction :output :if-exists :supersede)
    ...
    ;; ((and (listp xs) (null xs)) nil)
    ;; (t (let ((tree (car xs)))
    ;; 	 (cond
    ;; 	   ((and (listp tree) (not (null tree)) (symbolp (car tree)) (eq (car tree) 'seq))
    ;; 	    (loop for i from 1 to (- (length tree) 1) do
    ;; 	      (let ((elem (nth i tree)))
    ;; 		(gen elem))))
